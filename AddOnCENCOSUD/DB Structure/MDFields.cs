            ...
            string[] validValues, string[] validDescription, string DefaultValue)
        {
            try
            {
                ...
                if (exists == false)
                {
                    ...
                    //if (LinkedTo == "OACT")
                    //{
                    //    int Iversion = Convert.ToInt32(Globals.oCompany.Version.ToString().Substring(0, 2));
                    //    if (Iversion > 91) oUserFieldsMD.LinkedSystemObject = SAPbobsCOM.BoObjectTypes.oChartOfAccounts;
                    //}
                    #region valid Values
                    if ((validValues != null))
                    {
